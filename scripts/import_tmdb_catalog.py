...
Run from the repo root with the venv active:
    python scripts/import_tmdb_catalog.py [--kind movie|tv|all] [--min-votes 100]
                                          [--imdb-above 5] [--workers 6] [--limit N]
    python scripts/import_tmdb_catalog.py --existing   # re-sync titles already in the DB

Only titles rated above --imdb-above on IMDb (default 5, strictly
greater) are imported -- checked against IMDb's ratings dataset right
...
async def sync_all(
    ...
    """
    Worker pool, one DB session per worker. A failed title is rolled back
    and retried once -- the likely cause under concurrency is two workers
    ...
    async def worker() -> None:
        ...
        async with AsyncSessionLocal() as db:
            ...
            while True:
                ...
                for attempt in (1, 2):
                    try:
                        if await sync(tmdb_id, accept=accept) is None:
                            skipped += 1
                        else:
                            done += 1
                        break
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        help="only import titles rated strictly above this on IMDb (default 5; pass -1 to disable)",
    )
    ...
    accept = None
    if not args.existing and args.imdb_above >= 0:
        ...
        print(f"loaded {len(ratings):,} IMDb ratings; importing titles rated above {args.imdb_above}", flush=True)
        accept = rating_above(ratings, args.imdb_above)

    kinds = ["movie", "tv"] if args.kind == "all" else [args.kind]
    for kind in kinds:
        await run(kind, args.min_votes, args.workers, args.limit, args.existing, accept)
